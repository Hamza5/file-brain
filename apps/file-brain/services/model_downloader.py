    ...
    async def download_model(
        ...
        Args:
            progress_callback: Async callback function for progress updates.
                Receives dict with: status, progress_percent, message, file, etc.
        ...
        try:
            ...
            logger.info(f"Downloading model {MODEL_NAME} from {HUGGINGFACE_REPO_ID}")

            # Download in a thread to avoid blocking the event loop
            def do_download():
                """Perform the actual download (blocking)"""
            ...
            download_path = await loop.run_in_executor(None, do_download)

            logger.info(f"Downloaded model to {download_path}")

            # The files are downloaded to models_dir/paraphrase-multilingual-mpnet-base-v2
            # We need to rename the folder to add the ts_ prefix
            source_path = self.models_dir / MODEL_NAME

            if source_path.exists():
                # Remove existing target if present (in case of partial previous download)
                if model_path.exists():
                    shutil.rmtree(model_path)

                # Rename to ts_ prefixed name
                source_path.rename(model_path)
                logger.info(f"Renamed model directory to {model_path}")
