...
const centerTextPlugin = {
  ...
  beforeDraw: function (chart: {
    config: {
      type: string;
      options: { plugins: { centerText?: { value: number } } };
    };
    ...
    const textColor =
      style.getPropertyValue("--text-color").trim() || "#495057";
...
export const HeroStats: React.FC = () => {
  ...
  const chartConfig = useMemo(() => {
    ...
    const TOP_COUNT = 5;
    const allFileTypes = Object.entries(stats.file_types).sort(
      (a, b) => b[1] - a[1]
    );
    const topFileTypes = allFileTypes.slice(0, TOP_COUNT);
    const otherCount = allFileTypes
      .slice(TOP_COUNT)
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <p
            style={{
              ...
              fontWeight: 700,
              color:
                stats?.totals.discovered && stats.totals.discovered > 0
                  ? "var(--primary-color)"
          ...
          >
            {stats && stats.totals.discovered > 0
              ? stats.totals.discovered.toLocaleString()
              : "—"}
          </p>
          {(!stats || stats.totals.discovered === 0) && (
            <p
        ...
        {stats &&
          status &&
          status.current_phase !== "verifying" &&
          stats.totals.discovered > 0 &&
          (stats.runtime.running ||
            stats.totals.indexed < stats.totals.discovered) && (
            <Card
              style={{
                textAlign: "center",
                transition: "all 0.2s ease",
            ...
            >
              ...
              <div style={{ marginBottom: "0.2rem" }}>
                ...
                >
                  {status
                    ? status.indexing_progress.toFixed(2)
                    : Math.round(
                        (stats.totals.indexed / stats.totals.discovered) * 100
                      )}
                  %
                </p>
              </div>
              ...
              >
                {stats.totals.indexed.toLocaleString()} of{" "}
                {stats.totals.discovered.toLocaleString()} files
              </p>
            </Card>
          )}
      </div>
      ...
      {(!stats || stats.totals.indexed === 0) && (
        ...
        >
          ...
          >
            {hasFoldersConfigured
              ? "Ready to Index Your Files"
              : "Get Started with File Brain"}
          </h3>
          <p
            style={{
              fontSize: "0.85rem",
          ...
          {hasFoldersConfigured ? (
            ...
            >
              <li>
                Enable the <strong>Monitor</strong> toggle to track changes
              </li>
              <li>
                Enable the <strong>Crawler</strong> toggle to start indexing
            ...
            >
              <li>
                ...
                />{" "}
                <strong>Settings</strong> icon in the top right
              </li>
              <li>Add folders you want to watch and index</li>
              <li>
                Enable the <strong>Monitor</strong> toggle to track file changes
              </li>
              <li>
                Enable the <strong>Crawler</strong> toggle to start indexing
              </li>
          ...
          >
            Once some files are indexed, you'll be able to search through them
            instantly!
          </p>
        </Card>
      )}
